...
\subsection{Classificaçãoutilizando Bayes e PCA}
...
A classificação com a função \textit{naive_bayes} com o conjunto de classes reduzido com MDS utiliza do trecho de código a seguir.
